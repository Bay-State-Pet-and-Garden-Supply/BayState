"""Run baseline Gemini consolidation tests and save evidence artifacts."""
...
def load_test_harness_module() -> types.ModuleType:
    ...
    spec = spec_from_file_location("baseline_test_harness", module_path)
...
def extract_generate_system_prompt(prompt_builder_path: Path) -> tuple[str, str]:
    source_text = prompt_builder_path.read_text(encoding="utf-8")
    source_lines = source_text.splitlines()
    source_excerpt = "\n".join(source_lines[218:281])
    ...
    return source_excerpt, template_match.group(1)
...
def main() -> int:
    ...
    shopsite_constants_path = repo_root / "apps/web/lib/shopsite/constants.ts"
    fixtures_path = (
        repo_root
        / "apps/web/lib/consolidation/__tests__/fixtures/test-product-groups.json"
    )
    evidence_root = repo_root / ".sisyphus/evidence/baseline"

    prompt_source_excerpt, system_prompt_template = extract_generate_system_prompt(
        prompt_builder_path
    )
    shopsite_pages = extract_shopsite_pages(shopsite_constants_path)
    ...
    metadata: dict[str, object] = {
        ...
        "prompt_builder_source": str(
            prompt_builder_path.relative_to(repo_root)
        ).replace("\\", "/"),
        "prompt_builder_excerpt_lines": {"start": 219, "end": 281},
        "prompt_builder_excerpt": prompt_source_excerpt,
        "rendered_system_prompt": system_prompt,
        "render_context": {
            "categories": [],
            "category_guidance": "No category values were provided.",
            "shopsite_pages_count": len(shopsite_pages),
        },
    }
