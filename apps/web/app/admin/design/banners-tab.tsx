...
export function BannersTab({ initialSettings }: BannersTabProps) {
  ...
  const handleAction = async (formData: FormData) => {
    ...
    if (!result.success) {
      toast.error(result.error || 'Failed to update campaign banner settings');
    } else {
      toast.success('Campaign banner settings updated successfully');
    }
  };

  return (
    <AdminCard variant="panel" className="border-border">
      <AdminCardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            <AdminCardTitle>Promotional Banners</AdminCardTitle>
        ...
        <form action={handleAction} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={addMessage} className="rounded-md">
          ...
          <div className="space-y-2">
            ...
            <Select value={variant} onValueChange={(val) => setVariant(val as 'info' | 'promo' | 'seasonal')}>
              <SelectTrigger id="variant" className="w-full">
                <SelectValue placeholder="Select banner style" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="info">Info (Dark)</SelectItem>
                <SelectItem value="promo">Promo (Amber)</SelectItem>
                <SelectItem value="seasonal">Seasonal (Green)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {enabled && messages.length > 0 && (
            <div className="space-y-2">
              ...
              <div className={`rounded-lg py-2.5 px-4 text-center text-sm font-medium ${variant === 'info' ? 'bg-primary text-white' : variant === 'promo' ? 'bg-amber-500 text-foreground' : 'bg-green-600 text-white' }`}>
                {messages[0].text}
                {messages[0].linkText && (
