...
interface CampaignBannerFormProps {
    initialSettings: CampaignBannerSettings;
}

export function CampaignBannerForm({ initialSettings }: CampaignBannerFormProps) {
    ...
    const [cycleInterval, setCycleInterval] = useState(initialSettings.cycleInterval / 1000); // Convert to seconds for UI
        ...
        <Card className="border border-zinc-950 rounded-none">
            <CardHeader>
                <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-none bg-brand-gold border border-zinc-950 shadow-[1px_1px_0px_rgba(0,0,0,1)]">
                        <Megaphone className="h-5 w-5 text-brand-burgundy" />
                    </div>
                    <div>
                        <CardTitle>Campaign Banner</CardTitle>
                        <CardDescription>
                            Control the cycling promotional banner at the top of the storefront
                        </CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <form action={updateCampaignBannerAction} className="space-y-6">
                    {/* Hidden input for messages JSON */}
