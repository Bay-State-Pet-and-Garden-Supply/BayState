...
export default async function AnalyticsPage({ searchParams }: PageProps) {
  ...
  const { data: metricsData } = await supabase.rpc('get_sales_metrics', {
  ...
  const { data: trendsData } = await supabase.rpc('get_sales_trends', {
    start_date: startDate.toISOString(),
    end_date: endDate,
    period: 'month',
    p_source: source,
  ...
  const { data: driftData } = await supabase.rpc('get_inventory_drift', { p_days: 7 });
  const { data: healthData } = await supabase.rpc('get_sync_health', { p_days: 30 });
  ...
  return (
    <AdminPageShell
      title="Reporting"
      description="Track sales, sync health, and inventory drift without losing the operational context of the rest of the admin."
