...
describe('ShopSiteClient', () => {
    ...
    describe('fetchProducts', () => {
      ...
      <products>
        ...
        it('requests the product XML endpoint without legacy field filtering', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true,
                text: () => Promise.resolve('<?xml version="1.0"?><products></products>'),
                arrayBuffer: () => Promise.resolve(Buffer.from('<?xml version="1.0"?><products></products>')),
            ...
            expect(requestUrl.pathname).toBe('/db_xml.cgi');
            expect(requestUrl.searchParams.get('clientApp')).toBe('1');
            expect(requestUrl.searchParams.get('dbname')).toBe('products');
            expect(requestUrl.searchParams.get('version')).toBe('15.0');
            expect(requestUrl.searchParams.has('fields')).toBe(false);
        });
    });
