...
describe('AI scraping credentials admin route', () => {
  ...
  it('returns statuses and defaults on GET', async () => {
    (getAIScrapingCredentialStatuses as jest.Mock).mockResolvedValue({
      ...
      openai: { provider: 'openai', configured: true, last4: '9876', updated_at: null },
    ...
    expect(body.statuses.deepseek.configured).toBe(true);
    ...
    expect(body.defaults.llm_provider).toBe('deepseek');
    expect(body.consolidationDefaults.llm_supports_batch_api).toBe(false);
  });
  ...
  it('stores new provider keys and defaults on POST', async () => {
    ...
    (getAIScrapingCredentialStatuses as jest.Mock).mockResolvedValue({
      deepseek: { provider: 'deepseek', configured: true, last4: 'abcd', updated_at: null },
    ...
    expect(setAIScrapingProviderSecret).toHaveBeenCalledWith('deepseek', 'deepseek-test', 'user-1');
