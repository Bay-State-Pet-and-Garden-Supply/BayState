...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto shadow-[4px_4px_0px_rgba(0,0,0,1)]"
    >
      <table
        data-slot="table"
        className={cn(
          "w-full caption-bottom text-sm border-2 border-zinc-950 border-separate border-spacing-0 bg-white",
          className
        )}
        {...props}
      />
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-zinc-50 data-[state=selected]:bg-zinc-100 transition-colors group",
        className
      )}
      {...props}
    />
