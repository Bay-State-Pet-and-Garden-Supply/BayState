...
export function WishlistButton({ productId, initialState = false }: { productId: string, initialState?: boolean }) {
    ...
    return (
        <Button
            variant="ghost"
            size="icon"
            className="h-9 w-9 rounded-full bg-card shadow-sm hover:bg-muted hover:scale-110 transition-all"
            onClick={onClick}
        >
            <Heart className={cn("h-5 w-5 transition-colors", active ? "fill-red-500 text-red-500" : "text-muted-foreground")} />
