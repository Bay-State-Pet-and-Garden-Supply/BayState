...
interface TestSkuPanelProps {
  configId: string;
  testSkus: ScraperTestSku[];
}

export function TestSkuPanel({ configId, testSkus }: TestSkuPanelProps) {
  ...
  return (
    <Card data-testid="test-sku-panel">
      <CardHeader>
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'test' | 'fake' | 'edge_case')} className="w-full">
          ...
          <form onSubmit={handleAddSku} className="flex gap-2 mb-4" data-testid="sku-add-form">
            <Input
              ...
              disabled={isAdding}
            />
            <Button type="submit" disabled={isAdding || !newSku.trim()} data-testid="add-sku-button">
