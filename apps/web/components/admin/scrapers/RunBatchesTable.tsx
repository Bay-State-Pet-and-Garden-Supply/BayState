...
const statusConfig = {
 ...
 label: 'PENDING', 
 icon: Clock, 
 color: 'bg-muted text-zinc-500 border-border',
 dot: 'bg-zinc-400'
 },
 running: { 
 label: 'RUNNING', 
...
export function RunBatchesTable({ chunks, selectedChunkId, onSelectChunk }: RunBatchesTableProps) {
 ...
 className="border-2 border-border font-bold uppercase tracking-tight hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all"
 >
 Clear Filter
 </Button>
 )}
 ...
 <tr 
 key={chunk.id} 
 className={`border-b-2 border-border hover:bg-muted transition-colors ${isSelected ? 'bg-amber-50' : ''}`}
 >
 <td className="px-4 py-4 border-r-4 border-border font-mono font-bold text-foreground">
 {chunk.chunk_index}
 </td>
 ...
 <Progress 
 value={progress} 
 className="h-2 border border-border rounded-none bg-muted" 
 />
 </div>
 </td>
 <td className="px-4 py-4 border-r-4 border-border">
