...
import { CheckCircle, AlertCircle, Loader2, XCircle, Clock3 } from 'lucide-react';
...
} from '@/lib/admin/migration/history-entry';
...
function upsertLogEntry(
  current: MigrationLogEntry[],
  nextEntry: MigrationLogEntry,
): MigrationLogEntry[] {
  const withoutExisting = current.filter((log) => log.id !== nextEntry.id);

  return [nextEntry, ...withoutExisting].sort(
...
export function MigrationHistory({ initialLogs }: MigrationHistoryProps) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('integration-sync-run-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload: any) => {
          ...
          const nextEntry = mapIntegrationSyncRunToMigrationLogEntry(
            payload.new as IntegrationSyncRunHistoryRow,
          );

          setLogs((current) => upsertLogEntry(current, nextEntry));
        },
      )
      .subscribe();
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>Canonical sync runs from integration_sync_runs</CardDescription>
      </CardHeader>
      <CardContent>
    ...
    case 'queued':
      return (
        <div className="flex items-center text-slate-600 text-xs font-medium">
          <Clock3 className="w-3 h-3 mr-1" /> Queued
        </div>
      );
    case 'running':
      return (
