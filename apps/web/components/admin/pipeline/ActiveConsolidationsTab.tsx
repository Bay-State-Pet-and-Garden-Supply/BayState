...
const STATUS_CONFIG: Record<string, { label: string; color: string; bgColor: string; icon: typeof Loader2 }> = {
    validating:   { label: 'Validating',   color: 'text-gray-700',   bgColor: 'bg-gray-100',   icon: Clock },
    pending:      { label: 'Pending',      color: 'text-gray-700',   bgColor: 'bg-gray-100',   icon: Clock },
    in_progress:  { label: 'In Progress',  color: 'text-blue-700',   bgColor: 'bg-blue-50',    icon: Loader2 },
    finalizing:   { label: 'Finalizing',   color: 'text-amber-700',  bgColor: 'bg-amber-50',   icon: Loader2 },
    ...
    expired:      { label: 'Expired',      color: 'text-red-700',    bgColor: 'bg-red-50',      icon: XCircle },
            ...
            <div className="flex items-start justify-between">
                <div className="flex-1">
                    ...
                    <p className="mt-1 text-xs text-gray-400">
                        Started {formatTimestamp(job.createdAt)} • {formatElapsed(job.createdAt)} ago
                    </p>
                </div>
                ...
                <div className="flex items-center gap-1">
                    ...
                    >
                        <RefreshCw className={`h-4 w-4 ${syncingId === job.id ? 'animate-spin' : ''}`} />
                    </Button>
...
function BatchHistoryCard({
    ...
    const applySummary = metadata.apply_summary as Record<string, unknown> | undefined;
    const qualityMetrics = metadata.quality_metrics as Record<string, number> | undefined;
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
            ...
            <div className="mt-1.5 flex items-center gap-3 text-xs text-gray-500">
                ...
                {job.completed_requests > 0 && (
                    <>
                        <span>•</span>
                        <span className="text-green-600">{job.completed_requests} success</span>
                    </>
                )}
                ...
                <div className="mt-3 space-y-2 border-t pt-3">
                    <div className="grid grid-cols-2 gap-3 text-xs">
                        <div>
                            ...
                            <span className="ml-1 font-mono text-gray-700">{job.openai_batch_id || job.id}</span>
                        </div>
                        {job.completed_at && (
                            <div>
                                <span className="text-gray-500">Completed:</span>
                                <span className="ml-1 text-gray-700">{formatTimestamp(job.completed_at)}</span>
                            </div>
                        )}
                    </div>
                    ...
                    {qualityMetrics && (
                        <div className="rounded-md bg-gray-50 p-2">
                            <p className="text-xs font-medium text-gray-700 mb-1">Quality Metrics</p>
                            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                                <span>Matched brands: {qualityMetrics.matched_brand_count ?? 0}</span>
                                <span>Unresolved brands: {qualityMetrics.unresolved_brand_count ?? 0}</span>
                                <span>Fields overwritten: {qualityMetrics.overwritten_field_count ?? 0}</span>
                                <span>Fields preserved: {qualityMetrics.preserved_existing_field_count ?? 0}</span>
                            </div>
                        </div>
                    )}
                    ...
                    {applySummary && (
                        <div className="rounded-md bg-green-50 p-2">
                            <p className="text-xs font-medium text-green-700 mb-1">Apply Summary</p>
                            <div className="flex items-center gap-3 text-xs text-green-600">
                                <span>{(applySummary.success_count as number) ?? 0} applied</span>
...
export function ActiveConsolidationsTab({ className }: ActiveConsolidationsTabProps) {
    ...
    const handleCancel = async (batchId: string) => {
        if (!confirm('Cancel this consolidation batch? This cannot be undone.')) return;

        setCancellingId(batchId);
        try {
            const res = await fetch(`/api/admin/consolidation/${batchId}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const handleApply = async (batchId: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/consolidation/${batchId}/apply`, { method: 'POST' });
            if (res.ok) {
                const data = await res.json();
    ...
    const handleSyncAll = async () => {
        setSyncingAll(true);
        try {
            const res = await fetch('/api/admin/consolidation/sync', { method: 'POST' });
            if (res.ok) {
                const data = await res.json();
                toast.success(`Synced ${data.synced_count ?? 0} batch${(data.synced_count ?? 0) !== 1 ? 'es' : ''}`);
                await Promise.all([fetchJobs(), fetchHistory()]);
            } else {
