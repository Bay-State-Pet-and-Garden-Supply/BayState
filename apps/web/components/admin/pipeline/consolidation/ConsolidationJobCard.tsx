    ...
    <div className="rounded-none border border-border bg-card p-5 transition-colors hover:bg-accent/5">
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div className="flex-1 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="font-black uppercase tracking-widest text-base text-foreground">
              {job.description || `Batch ${job.id.slice(0, 8)}`}
            </h3>
            <StatusBadge status={job.status} />
            {llmModel && (
              <Badge variant="secondary" className="rounded-none border border-border bg-muted font-black uppercase text-[10px] h-5 tracking-widest">
                <Cpu className="mr-1 h-3 w-3" />
                {llmModel}
              </Badge>
            )}
          </div>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-black uppercase text-muted-foreground tracking-widest">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isTerminalStatus(job.status) && (
            <Button
              ...
              className="h-8 rounded-none border border-destructive text-[10px] font-black uppercase text-destructive hover:bg-destructive/5 tracking-widest"
            >
              {cancellingId === job.id ? (
      ...
      <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="rounded-none border border-border p-3 bg-blue-50/20">
          ...
          <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Processed</p>
        </div>
        <div className="rounded-none border border-border p-3 bg-status-success/10">
          ...
          <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Success</p>
        </div>
        <div className="rounded-none border border-border p-3 bg-destructive/10">
          <p
            className={`text-2xl font-black ${job.errorCount > 0 ? "text-destructive" : "text-muted-foreground"}`}
          >
            {job.errorCount}
          </p>
          <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Errors</p>
        </div>
      </div>

      {/* Error Warning */}
      {job.errorCount > 0 && (
        <div className="mt-3 flex items-center gap-2 rounded-none border border-destructive bg-destructive/5 px-3 py-2 text-[10px] font-black uppercase text-destructive tracking-widest">
