...
async function main() {
  ...
  try {
    ...
    profiles?.forEach((profile) => {
      if (profile.email) profileIdMap.set(profile.email.toLowerCase(), profile.id);
    });
    ...
    for (const [batchIndex, orderBatch] of orderBatches.entries()) {
      try {
        ...
        const sourceRecordRows = transformedBatch
          .map(({ sourceOrderNumber, transformedOrder }) => {
            const orderId = orderIdByNumber.get(transformedOrder.legacy_order_number);
        ...
        const eventRows = transformedBatch
          .map(({ transformedOrder }) => {
            const orderId = orderIdByNumber.get(transformedOrder.legacy_order_number);
            if (!orderId) return null;
            return {
              order_id: orderId,
        ...
        if (eventRows.length > 0) {
          const orderIds = eventRows.map((event) => event.order_id);
          ...
          const existingOrderIds = new Set((existingEvents || []).map((event) => event.order_id));
          const newEventRows = eventRows.filter((event) => !existingOrderIds.has(event.order_id));
        ...
        console.error(`Failed to process order batch ${batchIndex + 1}:`, message);
    ...
    if (syncRunId) {
      await completeIntegrationSyncRun(supabase, syncRunId, finalResult, {
        start_order: startOrderArg ?? null,
    ...
    console.error('Fatal sync error:', syncError instanceof Error ? syncError.message : syncError);
