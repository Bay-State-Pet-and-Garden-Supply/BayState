/**
 * Archive Script for scraper_runs (or scrape_jobs) table
 * 
 * Exports all data from the scraper runs table to NDJSON format
 ...
 * 
 * Output:
 *   .sisyphus/archive/scraper_runs_YYYYMMDD.ndjson
 */
...
const TABLE_NAME = 'scraper_runs'; // Will fallback to scrape_jobs if not found
...
async function main() {
  console.log('🗄️  Scraper Runs Archive Script');
  console.log('================================\n');
  
  const supabase = await getSupabaseAdmin();
  
  // Try scraper_runs first, then fallback to scrape_jobs
  let tableName = 'scraper_runs';
  let result = await exportTable(supabase, tableName);
  
  if (!result || result.rowCount === 0) {
    console.log(`\n⚠️  ${tableName} not found or empty, trying scrape_jobs...`);
    tableName = 'scrape_jobs';
    result = await exportTable(supabase, tableName);
  }
  
  if (!result || result.rowCount === 0) {
    console.log('\n❌ No scraper run data found to archive.');
    process.exit(0);
  }
