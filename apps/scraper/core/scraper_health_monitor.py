...
class ScraperHealthMonitor:
    ...
    def __init__(
        ...
        self._api_client = ScraperAPIClient()
    ...
    def register_scraper(self, scraper_name: str) -> ScraperDiagnostics:
        ...
        with self._lock:
            if scraper_name not in self._diagnostics:
                ...
                diagnostics.config_path = f"api://internal/scraper-configs/{slug}"
                ...
                try:
                    if self._api_client.api_url and self._api_client.api_key:
                        ...
                        if isinstance(payload, dict):
                            ...
                            diagnostics.has_test_skus = isinstance(test_skus, list) and len(test_skus) > 0
                    else:
    ...
    def record_run_success(
        ...
        with self._lock:
            ...
            if diagnostics.recent_runs > 0:
                diagnostics.recent_success_rate = diagnostics.recent_successes / diagnostics.recent_runs
    ...
    def record_run_failure(
        ...
        with self._lock:
            ...
            diagnostics.failure_types[failure_key] = diagnostics.failure_types.get(failure_key, 0) + 1

            # Update most common failure
            if diagnostics.failure_types:
                diagnostics.most_common_failure = max(diagnostics.failure_types.items(), key=lambda x: x[1])[0]
    ...
    def get_scraper_trend(self, scraper_name: str, hours: int = 24) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            first_success_rate = sum(1 for r in first_half if r["success"]) / len(first_half) if first_half else 0
            second_success_rate = sum(1 for r in second_half if r["success"]) / len(second_half) if second_half else 0
            ...
            return {
                ...
                "avg_duration": round(sum(r.get("duration", 0) or 0 for r in recent) / len(recent), 2),
                "trend": trend,
    ...
    def get_recommendations(self, scraper_name: str) -> list[str]:
        ...
        with self._lock:
            ...
            if diagnostics.success_rate < 0.5:
                recommendations.append("Critical: Success rate below 50%. Review error logs and check for site changes or blocking.")

            if diagnostics.circuit_breaker_open:
                recommendations.append("Circuit breaker is open. Wait for cooldown or investigate and reset manually.")
            ...
            if diagnostics.most_common_failure:
                ...
                if failure in ["rate_limited", "RATE_LIMITED"]:
                    recommendations.append("High rate limiting. Increase delays between requests and consider implementing request throttling.")
                elif failure in ["captcha_detected", "CAPTCHA"]:
                    recommendations.append("Frequent CAPTCHA detection. Implement CAPTCHA solving or adjust scraping patterns.")
                elif failure in ["access_denied", "ACCESS_DENIED"]:
                    recommendations.append("Access denied errors. Rotate user agents, implement session management, or use proxy rotation.")
                elif failure in ["element_not_found", "ELEMENT_NOT_FOUND"]:
                    recommendations.append("Element not found errors. Verify selectors are up-to-date with current site structure.")
                elif failure in ["timeout", "TIMEOUT"]:
                    recommendations.append("Timeout errors. Increase timeout settings or check network connectivity.")

            # Duration recommendations
            if diagnostics.avg_duration_seconds > 300:  # > 5 minutes
                recommendations.append(
                    f"Average run duration is {diagnostics.avg_duration_seconds:.0f}s. Consider optimizing selectors or reducing page loads."
                )
    ...
    def _update_duration_metrics(self, diagnostics: ScraperDiagnostics, duration: float) -> None:
        ...
        if diagnostics.total_runs == 1:
            ...
            # Rolling average
            diagnostics.avg_duration_seconds = ((diagnostics.avg_duration_seconds * (diagnostics.total_runs - 1)) + duration) / diagnostics.total_runs
    ...
    def _generate_status_change_alert(
        ...
        if new_severity > old_severity:
            ...
            if new_status == HealthStatus.CRITICAL:
                alert_type = "critical"
                severity = "critical"
                message = f"Scraper '{scraper_name}' is in CRITICAL state. Immediate attention required."
