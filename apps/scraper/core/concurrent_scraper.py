...
class ConcurrentScraperManager:
    ...
    def execute(
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                futures: dict[Future[ScrapingTask], ScrapingTask] = {}
    ...
    def _execute_task(
        ...
        logger.debug(
            "Task status pending",
            extra={"sku": task.sku, "scraper_name": task.scraper_name, "status": "pending"},
        )
        ...
        try:
            ...
            with self._throttler.acquire(timeout=60.0) as acquired:
                ...
                logger.debug(
                    "Task status scraped",
                    extra={"sku": task.sku, "scraper_name": task.scraper_name, "status": "scraped"},
                )
