...
├── configs/               # Deprecated local YAML references (API is runtime source)
├── context.py             # ScraperContext Protocol
├── models/                # Pydantic models
│   ├── config.py          # ScraperConfig
│   └── result.py          # ScrapeResult
└── parser/
    └── yaml_parser.py     # API config payload parsing/validation helper
```
...
- **NO** hardcoded site logic (use API-published config params)
