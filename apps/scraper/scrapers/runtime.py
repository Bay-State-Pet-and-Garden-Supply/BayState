...
def run_scraping(
    ...
    **kwargs,
    ...
    scraper_queues: dict[str, Queue[str]] = {}  # config.name -> Queue of SKUs
    tasks = []
    workers_used = 0
    ...
    def process_scraper(args: tuple[Any, ...]) -> dict[str, Any] | tuple[int, int]:
        ...
        while True:
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    if outcome == "success":
                        ...
                        emitter.data_synced(
                            sku=sku,
                            scraper=config.name,
                            data={"status": "no_results" if is_passing else "not_found"},
                        )
                        ...
                        emitter.data_synced(sku=sku, scraper=config.name, data={"status": "not_found"})
                else:
                    scraper_failed += 1
                    fake_skus_list = getattr(config, "fake_skus", []) or []
                    sku_type = "fake" if sku in fake_skus_list else "test"
                    ...
                    emitter.data_sync_failed(sku=sku, scraper=config.name, error="error")
                ...
                emitter.data_sync_failed(sku=sku, scraper=config.name, error=str(e))
    ...
    if test_mode and available_sites:
        try:
            ...
            for s_name, details in scraper_results_map.items():
                ...
                emitter.info(
                    f"Test summary generated for {s_name}",
                    data={"scraper": s_name, "summary": final_result.get("summary", {})},
                )
                ...
                emitter.info(
                    f"Calculated test health for {s_name}: {health_status}",
                    data={
                        "scraper": s_name,
                        ...
                        "test_skus_total": int((final_result.get("summary") or {}).get("total", 0)),
                    },
                )
                log(f"Updated health status for {s_name}: {health_status}", "INFO")

        except Exception as e:
            log(f"Failed to build test summary telemetry: {e}", "ERROR")
    ...
    if test_mode:
        ...
        log("Data synced via ResultCollector", "INFO")
