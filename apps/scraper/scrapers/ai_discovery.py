    ...
    def _validate_extraction_match(
        ...
        if sku:
            ...
            if sku and sku.lower() not in combined:
                has_strong_signals = confidence >= 0.8 and extracted_brand and brand and self._is_brand_match(brand, extracted_brand, source_url)
                if not has_strong_signals:
                    logger.warning(
                        f"[Discovery Validation] REJECTED: SKU not found and weak match signals "
                        f"(confidence={confidence:.2f}, brand_match={self._is_brand_match(brand, extracted_brand, source_url)})"
                    )
