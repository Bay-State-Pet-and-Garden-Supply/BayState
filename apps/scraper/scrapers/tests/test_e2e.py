...
class TestScraperIntegration:
    ...
    def available_scrapers(self, tester):
        ...
        if not scrapers:
            pytest.skip("API scraper list unavailable (SCRAPER_API_URL/SCRAPER_API_KEY missing)")
        return scrapers
    ...
    async def test_scraper_no_results_parametrized(self, tester, scraper_name):
        ...
        if not tester.api_client.api_url or not tester.api_client.api_key:
            pytest.skip("API credentials required for API-only scraper execution")
    ...
    def test_no_results_integration_with_config_validation(self, tester):
        ...
        api_client = tester.api_client
        if not api_client.api_url or not api_client.api_key:
