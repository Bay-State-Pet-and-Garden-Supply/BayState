...
def test_query_builder_identifier_queries_use_raw_sku_only() -> None:
    ...
    # 12-digit numeric UPC (UPC)
    query = qb.build_identifier_query(upc="813347002015")
    assert query == "813347002015"

    # Short numeric UPC
    query = qb.build_identifier_query(upc="12345")
    assert query == "12345"

    # Alphanumeric UPC
    query = qb.build_identifier_query(upc="ABC-123")
    assert query == "ABC-123"

def test_query_builder_search_query_prefers_product_name() -> None:
    qb = QueryBuilder()
    query = qb.build_search_query(upc="813347002015", product_name="Stud Muffins", brand="Manna Pro")

    assert query == "Stud Muffins"

def test_query_variants_return_name_follow_up_only() -> None:
    qb = QueryBuilder()
    variants = qb.build_query_variants(upc="813347002015", product_name="Stud Muffins", brand="Manna Pro", category=None)
