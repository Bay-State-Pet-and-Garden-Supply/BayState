...
BackgroundTasks = fastapi_module.BackgroundTasks
Depends = fastapi_module.Depends
FastAPI = fastapi_module.FastAPI
HTTPException = fastapi_module.HTTPException
Query = fastapi_module.Query

fastapi_cors_module = importlib.import_module("fastapi.middleware.cors")
...
class JobState:
    ...
    def __init__(self):
        ...
        self.worker_stats: dict[str, dict[str, Any]] = {}  # worker_id -> {scraper, current_sku, completed, failed}
...
def run_scraper_job(
    ...
    try:
        ...
        def scraper_progress_callback(data: dict[str, Any]):
        ...
        def debug_callback(data: dict[str, Any]):
...
async def list_scrapers():
    ...
    if not client.api_url:
        raise HTTPException(status_code=500, detail="SCRAPER_API_URL not configured")
    if not client.api_key:
        raise HTTPException(status_code=500, detail="SCRAPER_API_KEY not configured")

    try:
        configs = client.list_published_configs()
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to fetch scrapers from API: {e}") from e
    ...
    for item in configs:
        ...
        if not slug:
            continue
        name = _normalize_scraper_identifier(slug)
        ...
        scrapers.append({"name": name, "display_name": display_name})
