...
class XPathExtractionStrategyWrapper:
    """Wrapper around crawl4ai's JsonXPathExtractionStrategy."""
    ...
    def extract(self, html: str) -> List[Dict[str, Any]]:
        """Extract data from HTML using the XPath strategy."""
        strategy: Any = self._create_strategy()
        return strategy.extract(html)

    @classmethod
    def from_yaml_selectors(cls, base_selector: str, selectors: Dict[str, Any]) -> "XPathExtractionStrategyWrapper":
    ...
    def _resolve_strategy_class() -> Any:
        ...
        for candidate in ("JsonXPathExtractionStrategy", "XPathExtractionStrategy"):
            strategy_cls = getattr(extraction_module, candidate, None)
            if strategy_cls is not None:
                return strategy_cls
        return _RegexXPathStrategy
...
class _RegexXPathStrategy:
    ...
    def extract(self, html: str) -> List[Dict[str, Any]]:
        ...
        blocks = [html]
        if base_selector == "//div[@class='product']":
        ...
        for block in blocks:
            ...
            for field in fields:
                ...
                xpath_match = re.match(r"^//([a-z0-9]+)\[@class='([^']+)'\]$", selector)
                if xpath_match:
                    tag = xpath_match.group(1)
                    css_class = xpath_match.group(2)
