...
34. `useRunnerPresence` can keep its public API while migrating to `useRealtimeChannel` by treating the shared hook as the source of channel lifecycle and status, then layering presence-state normalization plus `channel.track()` calls on top.
35. To support presence consumers without reintroducing per-hook Supabase channel creation, `useRealtimeChannel` needs pooled presence-sync listener dispatch in the same place it already fans out broadcast messages and status updates.
